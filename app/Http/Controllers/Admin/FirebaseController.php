...
class FirebaseController extends Controller
{
    ...
    public function showNotificationForm()
    {
        ...
        return view('admin.notifications.send', compact('users', 'userGroups', 'usersWithTokens'));
    }
    ...
    public function sendToAllUsers(Request $request)
    {
        ...
        $totalUsers = $userResult['summary']['total_users'] ?? 0;
        $totalSaved = $userResult['summary']['saved_to_database'] ?? $userResult['summary']['total_users'] ?? 0;
        $totalPushSuccessful = $userResult['summary']['push_successful'] ?? $userResult['summary']['successful'] ?? 0;
        $totalPushFailed = $userResult['summary']['push_failed'] ?? $userResult['summary']['failed'] ?? 0;
