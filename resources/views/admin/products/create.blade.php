...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.select-variation-image-btn', function() {
            currentMediaTarget = 'variation_image';
            currentVariationIndex = $(this).data('variation-index');
            loadMediaLibrary();
        });
        
        $(document).on('click', '[data-bs-target="#mediaLibraryModal"][data-target="main_photo"]', function() {
            currentMediaTarget = 'main_photo';
            currentVariationIndex = null;
            loadMediaLibrary();
        });
        
        $(document).on('click', '[data-bs-target="#mediaLibraryModal"][data-target="gallery"]', function() {
            currentMediaTarget = 'gallery';
            currentVariationIndex = null;
            loadMediaLibrary();
        });
        
        // Load Media Library
        function loadMediaLibrary(append = false) {
            if (!append) {
                mediaPage = 1;
                $('#media-library-items').html(`
                    <div class="col-12 text-center py-5" id="media-loading">
                        <div class="spinner-border text-primary" role="status">
                            <span class="visually-hidden">Loading...</span>
                        </div>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.media && response.media.data) {
                        if (response.media.data.length === 0 && !append) {
                            $('#no-media-message').removeClass('d-none');
                        } else {
                            $('#no-media-message').addClass('d-none');
                            
                            if (!append) {
                                $('#media-library-items').empty();
                            }
                            
                            response.media.data.forEach(function(item) {
                                const mediaHtml = `
                                    <div class="col-md-2 col-sm-3 col-4">
                                        <div class="media-item card h-100 border-0 shadow-sm" 
                                             data-media-id="${item.id}" 
                                             data-media-url="${item.url}"
                                             style="cursor: pointer;">
                                            <div class="ratio ratio-1x1">
                                                ${item.mime_type.startsWith('image/') ? 
                                                    `<img src="${item.url}" class="card-img-top object-fit-cover" alt="${item.name}">` :
                                                    `<div class="d-flex align-items-center justify-content-center bg-light">
                                ...
                                `;
                                $('#media-library-items').append(mediaHtml);
                            });
                            
                            mediaHasMore = response.media.current_page < response.media.last_page;
                            if (mediaHasMore) {
                ...
                error: function(xhr) {
                    ...
                    $('#media-loading').html('<div class="text-danger">Error loading media</div>');
                }
            });
        }
        ...
        $(document).on('click', '.media-item', function() {
            $('.media-item').removeClass('border-primary border-3');
        ...
        $('#select-media-btn').on('click', function() {
            const selectedMedia = $('.media-item.border-primary');
            if (selectedMedia.length === 0) return;
            
            const mediaId = selectedMedia.data('media-id');
            const mediaUrl = selectedMedia.data('media-url');
            
            selectMedia(mediaId, mediaUrl, currentMediaTarget);
        });
        
        // Select Media Function
        function selectMedia(id, url, target) {
            if (target === 'variation_image' && currentVariationIndex !== null) {
                // Find the variation container
                const $container = $(`.variation-image-upload`).eq(currentVariationIndex);
                
                // Update preview
                $container.find('.variation-image-preview-container').html(`
                    <div class="position-relative">
                        <img src="${url}" 
                             class="img-fluid rounded variation-image-preview" 
                             style="max-height: 150px; object-fit: contain;" 
                             alt="Variation Image">
                        <button type="button" 
                                class="btn btn-danger btn-sm position-absolute top-0 end-0 m-1 remove-variation-image" 
                                title="Remove Image">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                ...
                // Set the image_id
                $container.find('.variation-image-id').val(id);
                ...
                $headerImg.html(`
                    <img src="${url}" 
                         ...
                         alt="Variation">
                `);
            } else if (target === 'main_photo') {
                // Handle main photo selection
                $('#main-photo-preview').html(`
                    <div class="position-relative">
                        <img src="${url}" class="img-fluid rounded" alt="Main Photo" style="max-height: 200px; object-fit: contain;">
                        <button type="button" class="btn btn-danger btn-sm position-absolute top-0 end-0 m-2 rounded-circle" onclick="removeMainPhoto()">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                `);
                ...
                // Handle gallery selection (add to gallery)
                const galleryItem = `
                    <div class="col-md-3 gallery-item" data-media-id="${id}">
                        <div class="position-relative">
                            <img src="${url}" class="img-fluid rounded" alt="Gallery Image" style="height: 150px; object-fit: cover; width: 100%;">
                            <button type="button" class="btn btn-danger btn-sm position-absolute top-0 end-0 m-2 rounded-circle remove-gallery-item">
                                <i class="fas fa-times"></i>
                            </button>
                        </div>
                    </div>
                `;
                $('#product-gallery-preview').append(galleryItem);
                updateGalleryInput();
            }
        ...
        $('#media-search').on('keyup', function() {
            mediaSearchQuery = $(this).val();
            loadMediaLibrary();
        });
        
        // Media Filter
        $('#media-filter').on('change', function() {
            mediaFilterType = $(this).val();
            loadMediaLibrary();
        });
        ...
        // Helper function to update gallery input
        function updateGalleryInput() {
            const galleryIds = [];
            $('.gallery-item').each(function() {
                galleryIds.push($(this).data('media-id'));
            });
            $('#product_gallery').val(JSON.stringify(galleryIds));
        }
